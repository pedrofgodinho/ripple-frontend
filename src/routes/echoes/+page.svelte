...
<main class="flex flex-wrap">
    {#each Object.keys($echoes) as key, i}
        <EchoEditor bind:echo={$echoes[key]} on:deleteEcho={(_) => removeEcho(key)}>Echo {i}</EchoEditor> <!-- Don't know how to get TS to accept key here, but it is guaranteed valid -->
    {/each}
